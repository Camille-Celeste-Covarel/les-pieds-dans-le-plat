...
const create = async (req: AuthRequest, res: Response) => {
  ...
  try {
    ...
    const baseSlug = `${slugify(author.public_name, { // On remplace le / par un -
      lower: true,
      strict: true,
    })}-${slugify(title, { lower: true, strict: true })}`;
    const finalSlug = `${baseSlug}-${Math.random()
      .toString(36)
      .substring(2, 8)}`;
...
const browse = async (req: AuthRequest, res: Response) => {
  try {
    ...
    const formattedPosts = posts.map((post) => {
      ...
      const plainTextContent = postJson.content.replace(/<[^>]*>?/gm, "");
...
const browseForAdmin = async (req: AuthRequest, res: Response) => {
  try {
    ...
    // Pas besoin de formater les URLs d'avatar ici, on garde la réponse simple
    res.json(posts);
  } catch (error) {
    console.error(
